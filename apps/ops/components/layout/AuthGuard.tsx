...
interface AuthGuardProps { children: React.ReactNode; requiredRole: UserRole; }

export function AuthGuard({ children, requiredRole }: AuthGuardProps) {
  const router = useRouter();
  const { user, profile, loading, logout } = useAuth();
  const { showIdleModal, dismissIdleModal } = useIdleTimer(() => logout().then(() => router.push('/login')));
  ...
  if (loading) return <div className="flex h-screen items-center justify-center"><Skeleton className="h-32 w-64" /></div>;
